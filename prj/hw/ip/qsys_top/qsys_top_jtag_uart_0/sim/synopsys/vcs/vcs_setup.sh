...
TOP_LEVEL_NAME="qsys_top_jtag_uart_0"
QSYS_SIMDIR="./../../"
...
source $QSYS_SIMDIR/common/vcs_files.tcl
set files [dict merge $files [qsys_top_jtag_uart_0::get_common_design_files "$QSYS_SIMDIR"]]
...
set QSYS_SIMDIR [lindex $argv 1]
source $QSYS_SIMDIR/common/vcs_files.tcl
if {[catch {qsys_top_jtag_uart_0::get_design_files "$QSYS_SIMDIR"} errorMsg]} {
 puts "Catch error: $errorMsg"
 exit 1 
}
set files [dict create]
set files [dict merge $files [qsys_top_jtag_uart_0::get_design_files "$QSYS_SIMDIR"]]
